...
@beam.ptransform_fn
def ReplaceCategoricalColumns(inputs, categorical_dict_rows):
    """
    ...
    categorical_mapping_dicts = categorical_dict_rows | beam.Map(
        lambda r: ((r[0], r[1]), r[2])
    )

    return inputs | beam.Map(
        lambda r, d: {
            column: d.get((column, old_value), old_value)
            for column, old_value in r.items()
        },
